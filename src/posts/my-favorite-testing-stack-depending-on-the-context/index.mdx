...
- [Stack for a product](#stack-for-a-product)
  * [Testing the transitioning states](#testing-the-transitioning-states)
...
When building a product, my main concern is to make sure the overall behavior is valid. I'm going to verify that the different pages create values by following business guidelines.
As part of this, I love writing 2 different kinds of tests (note that naming testing categories is not my business, I let you decide what they are).

### Testing the transitioning states
...
- verify the **fulfilled** state of the page, when the data has been resolved

For this kind of verification, I like to rely on [Jest](https://jestjs.io/fr/docs/getting-started), [Testing Library](https://testing-library.com/docs/react-testing-library/intro/) and [MSW](https://mswjs.io/).
...
I'm testing these transitioning states with these tools (and not Cypress) for speed reasons. [While experimenting on my machine](https://github.com/mfrachet/tests), Jest tests (from a single file, no parallelization) were ~8 times faster than their Cypress concurrent, and they require way less setup on the CI.

### Testing the E2E integration

It corresponds to starting the whole application, from the frontend to the database, passing by the backend. The idea is to verify the overall coherence of the data flow from one end to the other by navigating, like a regular user on the different pages of the app.

This kind of test takes time to run, they are hard to set up, but they provide a lot of confidence. From my experience, the earlier in the project process they are set up, the easier they are to maintain. Since they are costly, it's important not to run too much of them and
to scope what is critical in the application process.

For this kind of test, I love [Cypress](https://www.cypress.io/). It's very fast, easy to set up on the frontend (I'm not talking about starting the backend and DB here) and the developer experience is a breath.

As mentioned above, using this tool, I will try to make sure the business is meaningful and that my implementations resolve business use cases. It also means that I will test only what is predictable: I shouldn't be able to simulate a backend error (and the error case is already covered by the previous section).

> Trick: To speed up Cypress tests, you can create a custom command for login that directly hit the API, without going through the whole UIs. Plus, if you have the opportunity, try to run the test on a SQLite database. It saved ~100 seconds of tests in one of my apps passing from ~130 seconds to only ~30 seconds.
...
When building UI libraries (for a design system for example) my main concern is to make sure the components' behaviors and visual representations are the same across multiple browsers.
...
**It implies accessibility verifications and styles verifications.**
...
Tools like [Chromatic](https://www.chromatic.com/) (or [Percy](https://percy.io/)) allow comparing two versions of a UI: a valid one, and a newly suggested one. If there are differences between them, the developers have to manually accept (or reject) the newly suggested one that will become (or not) the valid one.
